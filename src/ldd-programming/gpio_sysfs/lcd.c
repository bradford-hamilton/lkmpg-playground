...
void lcd_display_clear(void)
{
  ...
  // Page 24 of datasheet - display clear cmd execution wait time is ~2ms
  usleep(2000); 
}
...
void lcd_display_return_home(void)
{
  ...
  // Page 24 of datasheet - return home cmd execution wait time is ~2ms
...
// Writes 4 bits of data/cmd to D4, D5, D6, D7 lines
static void write_4_bits(uint8_t data)
{
...
// Call this function to make LCD latch the data lines in to its internal registers.
...
{ 
  ...
  usleep(1);
  gpio_write_value(GPIO_LCD_EN, LOW_VALUE);
  // Execution time > 37 micro seconds
  usleep(100);
}
